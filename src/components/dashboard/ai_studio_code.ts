--- START OF FILE CampaignManager.tsx.txt ---
...
// import { Calendar } from "lucide-react"; // Calendar icon is already imported from lucide-react
...
interface Ad {
  ...
  campaign_id: string;
  name: string;
  ad_type: string; // 'image', 'video', 'text'
  headline: string;
  description: string;
  image_url?: string;
  video_url?: string;
  link_url?: string; // Ensured link_url is present
  budget: number;
  status: string;
  created_at: string;
}
  ...
  const fetchCampaignAds = async (campaignId: string) => {
    ...
    try {
      const { data, error } = await supabase
        .from('ads')
        .select('*') // link_url will be included with '*'
  ...
  const toggleCampaignStatus = async (campaignId: string) => {
    ...
    if (newStatus === 'active') {
      const { data: adsData, error: adsError } = await supabase
        .from('ads')
        .select('id, ad_type, image_url, video_url, headline, description, link_url')
        .eq('campaign_id', campaignId)
        .eq('user_id', user.id);

      if (adsError) {
        toast({
          title: "Error checking ads",
          description: adsError.message || "Failed to retrieve ad data for validation.",
          variant: "destructive",
        });
        return;
      }
      ...
      if (!adsData || adsData.length === 0) {
        toast({
          title: "No ads found",
          description: "Please create ads with content before activating the campaign.",
          variant: "destructive",
        });
        return;
      }

      // Validate ad content based on ad_type
      const adsWithContent = adsData.filter(ad => {
        // All ad types must have a headline, description, and link_url
        if (!ad.headline || !ad.description || !ad.link_url) {
          return false;
        }
        // Image ads must have an image_url
        if (ad.ad_type === 'image' && !ad.image_url) {
          return false;
        }
        // Video ads must have a video_url
        if (ad.ad_type === 'video' && !ad.video_url) {
          return false;
        }
        // Text ads are considered valid if they have headline, description, and link_url
        return true;
      });
      ...
      if (adsWithContent.length === 0) {
        toast({
          ...
          description: "All ads must have a headline, description, and a destination link. Image/Video ads also require their respective media.",
          variant: "destructive",
        });
        return;
      }
                                      ...
                                      <div>
                                        ...
                                        <div className="flex items-center gap-2">
                                          <a
                                            ...
                                            onClick={(e) => e.stopPropagation()} // Prevent collapsible from closing
                                          >
                                            {ad.link_url || 'No link URL set'}
                                          </a>
                                          {ad.link_url && (
                                            <Button
                                              size="sm"
